...
if ngens~=0
  for i=1:ngens
    ...
    % consistency quadratic hessian blocks:
    if ngens~=0
        VarVal1 = PuScale*rho_theta*sigma;
        VarVal2 = PuScale*rho_V*sigma;
    else
        VarVal1 = rho_theta*sigma;
        VarVal2 = rho_V*sigma;
    end

    % cw^2 scaling ONLY for COPY variables in primal mode (paper)
    if admm_mode == "primal"
    ...
    for i=1:regvar
        ...
        % copy vars: angle copy index = 2*nb+2*ng+2*i, mag copy index = 2*nb+2*ng+2*i-1
        H(2*nbuses+2*ngens+(2*i),   2*nbuses+2*ngens+(2*i))   = H(2*nbuses+2*ngens+(2*i),   2*nbuses+2*ngens+(2*i))   + cw2*(VarVal1/theta_scale)/theta_scale;
        H(2*nbuses+2*ngens+(2*i-1), 2*nbuses+2*ngens+(2*i-1)) = H(2*nbuses+2*ngens+(2*i-1), 2*nbuses+2*ngens+(2*i-1)) + cw2*(VarVal2/V_scale)/V_scale;
    end
