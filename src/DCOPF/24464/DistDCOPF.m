...
gamma1    = 1.0;   % finite/fixed gain (gamma_1 < 0)
gamma2    = 0.85;   % fixed-time extra gain (gamma_2 < 0)
...
for regionIdx = 1:numRegions
    ...
    PgWarmName    = ['Pg_warm_' regionID];
    DeltaWarmName = ['Delta_warm_' regionID];

    % First iteration can use zeros, or a better feasible guess if you want
    assignin('base', PgWarmName, zeros(size(regionData.gen,1),1));
    assignin('base', DeltaWarmName, zeros(size(regionData.bus,1) + nitLocal,1));
