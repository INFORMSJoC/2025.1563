...
% Compatible with DistDCOPF.m (extended version)
%
...
%   - theta_scale      : scaling for angle residuals
%   - mode             : 'none' | 'primal' | 'dual'
%   - updateType       : 'asy' | 'finite' | 'fixed' (for primal/dual modes)
%   - w1, w2           : weights for primal mode (ignored otherwise)
%
% Region-specific data from 'base' workspace:
%   - nbR#, nitR#, conR#, YonR#, region_R#, phase_angles_R#, hat_phase_angles_R#
%
% On exit, leaves:
%   - Pg               : sdpvar(#gen,1)
...
hasRTE    = contains(case_name, 'rte', 'IgnoreCase', true); %#ok<NASGU>

% region-specific tie-line descriptor cell (as built in DistDCOPF)
dataMatrix = loadedData.(tie_var);

% Sort tie-line rows so that region1_num < region2_num (consistent)
...
% NEW: auxiliary boundary angles for primal mode
hatPhase = [];
if strcmpi(mode, 'primal')
    hatName = ['hat_phase_angles_' reg];
    if evalin('base', sprintf('exist(''%s'',''var'')', hatName))
        hatPhase = evalin('base', hatName);
    else
        hatPhase = zeros(nitq, 1);
        assignin('base', hatName, hatPhase);
    end
end
...
Rgrad      = [];  % primal consistency residuals (first angle pair)
Rgrad_dual = [];  % "dual" residuals (second angle pair)

usePrimalAccel = strcmpi(mode, 'primal');
useWeighted    = usePrimalAccel && (w1 + w2 > 0);
...
for i = 1:nitq
    ...
    % conq(i) = local interface index in neighbor region
    ...
    % Weighted remote angle for primal-accel mode
    if useWeighted
        hat_i = hatPhase(i);
        copy_w = (w1 * Delta(remote_bus) + w2 * hat_i) / (w1 + w2);
    else
        copy_w = Delta(remote_bus);
    end
    ...
    if val_num == region1
        ...
        %
        % For primal mode, only the "remote" side uses weighted copy.
        Rgrad = [Rgrad; Delta(local_bus) - angle_1];

        if useWeighted
            Rgrad_dual = [Rgrad_dual; copy_w - angle_2];
        else
...
% --- Penalty scaling based on mode and updateType ----------------------
if strcmpi(mode, 'primal') 
    PScale = 2.5;              % primal finite/fixed
elseif strcmpi(mode, 'dual') 
    PScale = 8;               % dual finite/fixed (different value)
else
    PScale = 1;                 % all other cases
end

if (strcmpi(mode, 'primal') && strcmpi(updateType, 'asy'))
PScale = 1.5;
end

if (strcmpi(mode, 'primal') && strcmpi(updateType, 'fixed'))
PScale = 5.0;
end

if (strcmpi(mode, 'dual') && strcmpi(updateType, 'fixed'))
PScale =10.0;
end
...
if ~isempty(Yonq)
    for k = 1:numel(Yonq)
        ...
        % Normalize local residuals by theta_scale (same as in DistDCOPF)
...
sol     = optimize(Constraints, Objective, options); %#ok<NASGU>

local_solver_time = sol.solvertime;   % time spent inside the solver
local_yalmip_time = sol.yalmiptime;   % time to build/parse problem


local_wall_time   = local_yalmip_time + local_solver_time;
